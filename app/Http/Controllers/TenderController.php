...
class TenderController extends Controller
{
    ...
    {
        // Filter out empty entries that Alpine sends for blank list items
        $request->merge([
            'deliverables'       => array_values(array_filter($request->input('deliverables', []), fn ($v) => is_string($v) && trim($v) !== '')),
            'special_conditions' => array_values(array_filter($request->input('special_conditions', []), fn ($v) => is_string($v) && trim($v) !== '')),
            'boq_items'          => array_values(array_filter($request->input('boq_items', []), fn ($v) => is_array($v) && trim($v['description'] ?? '') !== '')),
        ]);
